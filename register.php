...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($flag == 0) {
        ...
        $addlogin1 = "INSERT INTO Block_Members(status ,block_id, user_id )VALUES( 'pending'," . $custrow1['block_id'] . ", " . $custrow['user_id'] . ")";
        mysql_query($addlogin1);
        $b = $custrow1['block_id'];
        $c = "select user_id from block_members where block_id = '$b' and status = 'member'";
        $xyz = mysql_query($c);
        while($row = mysql_fetch_array($xyz)){
            $temp = $row['user_id'];
            mysql_query("insert into block_approval values( '$t','$temp','$b','pending')");
        }
